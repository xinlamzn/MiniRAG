"""
Test script: MiniRAG with Bedrock LLM + OpenSearch docgraph storage.

Prerequisites:
  - OpenSearch running at https://localhost:9200 (with admin:Admin@1234)
  - Graph plugin with docgraph enabled: plugins.graph.docgraph.enabled=true
  - AWS credentials configured (IAM role or env vars) with Bedrock access
  - pip install aioboto3

Usage:
  python test_bedrock_docgraph.py
"""
...
os.environ["OPENSEARCH_DATABASE"] = "minirag_docgraph_test"
...
WORKING_DIR = "./minirag_docgraph_test_workdir"
...
async def cleanup_indices(session):
    ...
    if isinstance(data, list):
        for idx in data:
            name = idx.get("index", "")
            if name.startswith("minirag_docgraph_test"):
...
async def refresh_all(session):
    """Refresh all minirag_docgraph_test* indices so doc counts are accurate."""
    await os_request(session, "POST", "minirag_docgraph_test*/_refresh")
...
async def check_indices(session):
    ...
    _, data = await os_request(
        session, "GET", "_cat/indices/minirag_docgraph_test*?h=index,docs.count&format=json"
    )
    if not isinstance(data, list) or not data:
        print("  ERROR: No minirag_docgraph_test indices found!")
        return False
...
async def check_graph(session):
    """Print sample graph nodes and edges via evidence-mediated Cypher."""
    print("\n=== Phase 4: Checking docgraph data ===")
    from minirag.kg.opensearch_impl import _graph_database_name
    db_name = _graph_database_name()
    # Count entities (via Chunk mediation)
    status, data = await os_request(
        session, "POST", "_plugins/_cypher",
        json_body={"query": "MATCH (c:Chunk)-[:MENTIONS]->(n:Entity) RETURN count(DISTINCT n) AS cnt", "database": db_name},
    )
    cnt = data.get("data", [{}])[0].get("cnt", 0) if status == 200 else f"? (HTTP {status})"
    print(f"  Entities: {cnt}")
    # Count RelFacts (via Chunk mediation)
    status, data = await os_request(
        session, "POST", "_plugins/_cypher",
        json_body={"query": "MATCH (c:Chunk)-[:ASSERTS]->(rf:RelFact) RETURN count(rf) AS cnt", "database": db_name},
    )
    cnt = data.get("data", [{}])[0].get("cnt", 0) if status == 200 else f"? (HTTP {status})"
    print(f"  RelFacts: {cnt}")
    # Count Documents and Chunks
    status, data = await os_request(
        session, "POST", "_plugins/_cypher",
        json_body={"query": "MATCH (d:Document) RETURN count(d) AS cnt", "database": db_name},
    )
    cnt = data.get("data", [{}])[0].get("cnt", 0) if status == 200 else "?"
    print(f"  Documents: {cnt}")
    status, data = await os_request(
        session, "POST", "_plugins/_cypher",
        json_body={"query": "MATCH (c:Chunk) RETURN count(c) AS cnt", "database": db_name},
    )
    cnt = data.get("data", [{}])[0].get("cnt", 0) if status == 200 else "?"
    print(f"  Chunks: {cnt}")
    # Sample entities
    status, data = await os_request(
        session, "POST", "_plugins/_cypher",
        json_body={"query": "MATCH (c:Chunk)-[:MENTIONS]->(n:Entity) RETURN properties(n) AS props LIMIT 5", "database": db_name},
    )
    if status == 200:
        for row in data.get("data", []):
            props = row.get("props", {})
            print(f"    Entity: {props.get('entity_id', '?')} (type={props.get('entity_type', '?')})")
...
async def main():
    ...
    try:
        ...
        rag = MiniRAG(
            ...
            graph_storage="OpenSearchDocgraphStorage",
